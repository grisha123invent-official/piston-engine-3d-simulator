...
| 🌍 **Два языка** | Русский и английский — переключаются на лету, включая теорию, графики и подписи деталей |
| 🧭 **Карта режимов** | Тепловая карта расхода по оборотам и нагрузке с изолиниями, зоной детонации и «островом экономичности» |
| ⚗️ **Циклы и смесеобразование** | Цикл Аткинсона, прямой впрыск, впуск переменной длины, балансирные валы |
...
Панели сделаны как матовое стекло: сильное размытие фона, тонкая светлая обводка,
мягкая тень — двигатель просвечивает сквозь них и остаётся главным на экране.
Типографика и элементы управления системные: сегментированные переключатели,
тумблеры и ползунки ведут себя как в macOS и iOS.

В шапке — круговой указатель угла коленвала с секторами тактов и живые показания
мощности, момента и оборотов. Быстрые режимы задают целый сценарий одним кликом,
остальные настройки убраны в сворачиваемые секции.
...
<img src="docs/mobile.png" alt="Мобильный вид" width="31%">
<img src="docs/mobile-menu.png" alt="Меню управления" width="31%">

Весь экран отдан 3D-модели: панели скрыты, подписи деталей выключены, камера сама
отодвигается под вертикальный экран. Управление и теория открываются шторками снизу —
бургер слева, «Теория» справа. Тап по сцене закрывает шторку.

Вместо панелей сверху остаётся компактная плашка: такт, угол, шкала цикла и три главных
значения — давление, температура, обороты. Тап по ней открывает управление.
...
## ⚗️ Цикл Аткинсона и прямой впрыск

<img src="docs/atkinson.png" alt="Цикл Аткинсона" width="100%">

**Цикл Аткинсона** — впускной клапан закрывается позже, и часть заряда выталкивается обратно
во впускной коллектор. Действительная степень сжатия падает, а степень расширения остаётся
геометрической — газы успевают отдать больше работы:

| Позднее закрытие | ε действ. | Выброшено назад | КПД замкнутого цикла | Мощность |
|---|---|---|---|---|
| 0° | 8,8 | 0 % | 40,4 % | 49,0 кВт |
| 30° | 7,0 | 24 % | 41,4 % | 34,5 кВт |
| 60° | 4,6 | 54 % | **43,2 %** | 15,0 кВт |

При неизменном дросселе эффективный КПД при этом падает — трение и насосный ход не уменьшаются
вместе с зарядом. Выигрыш появляется **при равной мощности**: на 2000 об/мин Аткинсон с поздним
закрытием 30° даёт удельный расход 284 против 289 г/(кВт·ч) у обычного цикла, задушенного
дросселем до той же мощности. Плюс детонация отступает: интеграл падает с 0,58 до 0,29,
и можно поднять геометрическую степень сжатия.

**Прямой впрыск** — топливо испаряется прямо в цилиндре и охлаждает заряд на 23,7 K.
Наполнение растёт с 90,9 до 93,8 %, мощность с 49,0 до 56,6 кВт, а предельная степень сжатия
без детонации поднимается с **12,5 до 13,7**.

## 🔩 Оппозитная четвёрка и балансирные валы

<img src="docs/boxer.png" alt="Оппозитная четвёрка" width="100%">

Противолежащие поршни ходят навстречу друг другу, у каждого — **своя шатунная шейка со сдвигом
180°**: именно это отличает боксёр от V-образника с развалом 180°. Порядок работы 1–3–2–4.

| Компоновка | 1-й порядок | 2-й порядок | Продольный момент |
|---|---|---|---|
| Рядная 4 | 0 Н | 2874 Н | 0 |
| **Оппозитная 4** | **0 Н** | **0 Н** | 74 Н·м |
| V8 90° | 0 Н | 0 Н | 886 Н·м |

**Балансирные валы** гасят то, что осталось: у одноцилиндрового первый порядок падает
с 2334 до 350 Н, у рядной четвёрки второй — с 2874 до 431 Н (валы Ланчестера, вращаются вдвое
быстрее коленвала). Полностью погасить нельзя — остаточные 15 % видно на графике.

## 🧭 Карта режимов

<img src="docs/chart-map.png" alt="Карта режимов" width="52%" align="right">

Сетка 24×16 по осям «обороты × нагрузка» считается за 0,4 секунды и показывает, где двигатель
экономичен, а где нет. Изолинии — удельный расход, красная штриховка — зона детонации,
кольцо — «остров экономичности» (242 г/(кВт·ч) при 3513 об/мин), перекрестье — текущий режим.
Переключается между расходом, мощностью и КПД.

Именно на этой карте видно, почему на низкой нагрузке расход в разы хуже: полезная работа
падает быстрее, чем трение и насосные потери.

<br clear="right">

## 🌍 Два языка

<img src="docs/en/power.png" alt="Английский интерфейс" width="100%">

Переключатель **RU / EN** в шапке пульта меняет язык всего на лету — панель, теорию,
все восемь графиков и подписи деталей в 3D. Выбор запоминается, а ссылка `?lang=en`
открывает симулятор сразу по-английски.
...
&atkinson=45    позднее закрытие впуска (цикл Аткинсона), градусов
&di=1           прямой впрыск
&varIntake=1    впуск переменной длины
&shafts=1       балансирные валы
&lang=en        язык интерфейса: ru | en
...
- [ ] Отключение цилиндров на малой нагрузке
- [ ] Рециркуляция отработавших газов и её влияние на детонацию
- [ ] Фазовращатели на распредвалах с плавным изменением фаз
- [ ] Расчёт выбросов: CO, NOx и сажа по режимам
- [ ] Свободный поршень и роторный двигатель для сравнения схем
